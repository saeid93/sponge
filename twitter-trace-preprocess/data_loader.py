            ...
            for i, line in enumerate(bzinput):
                ...
                if len(tweets)> 0:
                    ...
                    if 'created_at' in tweets.keys():
                        time = datetime.datetime.fromtimestamp(int(tweets['timestamp_ms'])//1000.0).__str__()
                    ...
                    if 'delete' in tweets.keys():
                        time = datetime.datetime.fromtimestamp(int(tweets['delete']['timestamp_ms'])//1000.0).__str__()
                        if time in time_series.keys():
                            time_series[time]+=1
                        else:
                            time_series[time] = 1
