...
def _get_input_parameters(payload: Union[RequestInput, ResponseOutput]):
    parameters = payload.parameters.dict()
    return parameters

def _extract_content_type(parameters: List[Dict[str, Any]]):
    content_types = list(map(lambda l: l['content_type'], parameters))
    if len(set(content_types)) > 1:
        raise ValueError('content_type of a batch should be similar')
...
class BatchedRequests:
    ...
    def _merge_request_inputs(
        ...
        for internal_id, request_input in request_inputs.items():
            ...
            custom_params.append(_get_input_parameters(request_input))
        ...
        all_params['custom_parameters'] = custom_params
        all_params['content_type'] = _extract_content_type(custom_params)
