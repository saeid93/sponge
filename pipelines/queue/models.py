...
class Queue(MLModel):
    ...
    async def predict(self, payload: InferenceRequest) -> InferenceResponse:
        ...
        if type(payload.inputs[0].parameters.extended_parameters) == list:
            payload.inputs[0].parameters.extended_parameters_repeated = payload.inputs[
                0
        ...
        if type(output.outputs[0].parameters.extended_parameters_repeated) == list:
            output.outputs[0].parameters.extended_parameters = output.outputs[
                0
            ].parameters.extended_parameters_repeated
