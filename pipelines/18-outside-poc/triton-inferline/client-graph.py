...
deployment_name = 'video-monitoring-test'
namespace = "default"
...
# batch = torch.stack(list(map(
#     lambda a: yolo_resize(transform(a)), list(images.values()))))
...
def predict(data):
    ...
    print(data.shape)
    r = requests.post(f"{URL}/v2/models/infer", json=payload)
