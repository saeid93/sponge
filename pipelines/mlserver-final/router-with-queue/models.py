...
try:
    BATCH_SIZES = json.loads(os.environ["BATCH_SIZES"])
    logger.info(f"MODEL_LISTS set to: {MODEL_LISTS}")
except KeyError as e:
    raise ValueError(f"BATCH_SIZES env variable not set!")
...
class Router(MLModel):
    async def load(self):
        ...
        self.queue_managers = [
            asyncio.create_task(self.queues[model_name], batch_size)
