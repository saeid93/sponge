...
def get_memory_usage(pod_name, name_space,end, need_max = False):
    ...
    query = f"container_memory_usage_bytes{{pod=~'{pod_name}.*', container='video-yolo', namespace='{name_space}'}}"
    if need_max:
        query = f"max_over_time(container_memory_usage_bytes{{pod=~'{pod_name}.*', container='video-yolo', namespace='{name_space}'}}[{end}m])"
