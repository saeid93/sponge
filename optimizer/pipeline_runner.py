...
def experiments(pipeline_name: str, node_names: str,
    ...
    print('\n')
    if workload_type == 'static':
        workload = [workload] * load_duration
    try:
...
def optimizer(pipeline_name):
    # TODO load optimizer
    pass


def adapter(pipeline_name):
    monitoring(pipeline_name=pipeline_name)
    optimizer(pipeline_name=pipeline_name)
    change_config(pipeline_name=pipeline_name, config=config)


def monitoring(pipeline_name):
...
def load_test(pipeline_name: str, data_type: str,
    ...
    # TODO load data in a separate function
    if data_type == 'audio':
